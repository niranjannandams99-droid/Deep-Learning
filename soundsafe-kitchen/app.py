...
# Add author information to the app footer and about page


# Ensure streamlit_dir is defined (directory containing this file)
try:
    streamlit_dir
except NameError:
...
YOUR_NAME  = "Niranjan Nandam"
YOUR_EMAIL = "niranjan.nandams99@gmail.com"  
GITHUB_URL = "https://github.com/niranjannandams99-droid/Deep-Learning/tree/main/soundsafe-kitchen"
APP_URL    = "https://soundsafe-kitchen.streamlit.app"

# Adding footer to the sidebar section
# Find the sidebar section and add contact info after the caption
old_sidebar_end = '''        st.caption("CP2 - Deep Learning Track")
        st.caption("Dataset: ESC-50 + Synthetic")'''

new_sidebar_end = f'''        st.caption("CP2 - Deep Learning Track")
        st.caption("Dataset: ESC-50 + Synthetic")
        st.divider()
        st.markdown("**Author**")
        st.markdown(f"👤 {YOUR_NAME}")
        st.markdown(f"📧 {YOUR_EMAIL}")
        st.markdown(f"[🐙 GitHub]({GITHUB_URL})")'''

if old_sidebar_end in content:
    content = content.replace(old_sidebar_end, new_sidebar_end)
    print('✓ Author info added to sidebar')
else:
    print('Sidebar section not found — adding manually')

# Save updated app
with open(app_path, 'w', encoding='utf-8') as f:
    f.write(content)

print(f'✓ app.py updated with your details')
